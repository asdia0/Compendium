...
\resph{https://www.youtube.com/watch?v=GhAYpBa6MkM}{Review by \textit{Way Tan}}
...
\sol{1} Setting $x=0$ and relabelling $y$ as $x$, we get \[P(0, x) = P(x, x) \tag{1}.\] Setting $y = x$, we get \[P(x, x) = P(2x , 0).\] Setting $y = 0$, we get \[P(x, 0) = P(x, -x).\] Setting $y = -x$, we get \[P(x, -x) = P(0, -2x).\] We hence have \[P(0, x) = P(x, x) = P(2x, 0) = P(2x, -2x) = P(0, -4x),\] implying \[P(0, x) = P(0, -\tfrac14 x) \tag{2}.\] From (1), we have $P(1, 1) = P(0, 1)$. Using (2) repeatedly, we have \[P(0, 1) = P(0, -\tfrac14) = P(0, \tfrac1{16}) = \cdots = P(0, 0) = 2020.\] Thus, $P(1, 1) = 2020$.

\sol{2}[Abusing uniqueness] Suppose $k > 2020$ is the largest possible value of $P(1, 1)$. Then $k$ can be as big as we wish it to be. However, by the nature of the problem, $k$ should be unique. Hence, $k > 2020$ is impossible, implying that $2020$ is indeed the largest possible value of $P(1, 1)$ (occurring when $P(x, y) = 2020$ for all $x$, $y$).
...
\begin{center}
    \begin{tikzpicture}[scale=0.7]
        ...
        \coordinate[label=above left:$P$] (P) at (-3.392, 0.002);
        \coordinate[label=above right:$Q$] (Q) at (3.286, 2.714);
        ...
        \tkzMarkSegment[pos=.5,mark=|](C,P);
        \tkzMarkSegment[pos=.5,mark=|](P,X);
        \tkzMarkSegment[pos=.5,mark=||](X,Q);
        \tkzMarkSegment[pos=.5,mark=||](Q,B);
